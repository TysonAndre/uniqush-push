...
func (self *RestAPI) pushNotification(reqId string, kv map[string]string, perdp map[string][]string, logger log.Logger, remoteAddr string, handler ApiResponseHandler) {
	...
	if err != nil {
		logger.Errorf("RequestId=%v From=%v Service=%v NrSubscribers=%v Subscribers=\"%+v\" NoPushType: %v", reqId, remoteAddr, service, len(subs), subs, err)
		handler.AddDetailsToHandler(ApiResponseDetails{RequestId: &reqId, From: &remoteAddr, Code: UNIQUSH_ERROR_CANNOT_GET_PUSHTYPE})
		return
	}
	...
	if notif.IsEmpty() {
		logger.Errorf("RequestId=%v From=%v Service=%v NrSubscribers=%v Subscribers=\"%+v\" PushType=%v EmptyNotification", reqId, remoteAddr, service, len(subs), subs, pushtype)
		handler.AddDetailsToHandler(ApiResponseDetails{RequestId: &reqId, From: &remoteAddr, Service: &service, Pushtype: &pushtype, Code: UNIQUSH_ERROR_EMPTY_NOTIFICATION})
		return
	}

	logger.Infof("RequestId=%v From=%v Service=%v NrSubscribers=%v Subscribers=\"%+v\"", reqId, remoteAddr, service, len(subs), subs)

	self.backend.Push(reqId, service, subs, notif, perdp, logger, handler)
}
